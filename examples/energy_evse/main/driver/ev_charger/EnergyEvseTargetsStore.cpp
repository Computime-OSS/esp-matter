...
template <typename T>
CHIP_ERROR AllocAndCopyDailyTargetsWithLog(ChargingTargetsMemMgr & mgr, const T & src)
{
    CHIP_ERROR const err = mgr.AllocAndCopy(src);
    if (err != CHIP_NO_ERROR)
    {
        PRINTF_DEBUG("SetTargets: Failed to copy the new chargingTargets %s", chip::ErrorStr(err));
    }
    return err;
}
...
CHIP_ERROR EvseTargetsDelegate::DecodeChargingTargetStructFromTlv(TLV::TLVReader & reader,
                                                                Structs::ChargingTargetStruct::Type & outTarget)
{
    CHIP_ERROR err;
    while ((err = reader.Next()) == CHIP_NO_ERROR)
    {
        TLV::TLVType const type = reader.GetType();
        TLV::Tag const tag      = reader.GetTag();
        ...
        {
            ReturnErrorOnFailure(reader.Get(outTarget.targetTimeMinutesPastMidnight));
            continue;
        }
        if (tag == TLV::ContextTag(TargetEntryTag::kTargetSoC))
        {
            chip::Percent tempSoC;
            ReturnErrorOnFailure(reader.Get(tempSoC));
            outTarget.targetSoC.SetValue(tempSoC);
            continue;
        }
        if (tag == TLV::ContextTag(TargetEntryTag::kAddedEnergy))
        {
            int64_t tempAddedEnergy;
            ReturnErrorOnFailure(reader.Get(tempAddedEnergy));
            outTarget.addedEnergy.SetValue(tempAddedEnergy);
            continue;
        }
        return CHIP_ERROR_UNEXPECTED_TLV_ELEMENT;
    }
...
{
    PRINTF_DEBUG("EVSE: Initializing EvseTargetsDelegate");
...
CHIP_ERROR EvseTargetsDelegate::LoadTargets()
{
    ...
    while ((err = reader.Next(TLV::kTLVType_Structure, TLV::AnonymousTag())) == CHIP_NO_ERROR)
    {
        ...
        PRINTF_DEBUG("LoadTargets: DayOfWeekForSequence = 0x%02x",
               ...
               CHIP_NO_ERROR)
        {
            ...
            ReturnErrorOnFailure(DecodeChargingTargetStructFromTlv(reader, chargingTarget));
        ...
        if (err != CHIP_NO_ERROR)
        {
            PRINTF_DEBUG("SetTargets: Failed to allocate memory during LoadTargets %s", chip::ErrorStr(err));
            return err;
        }
    ...
    const DataModel::DecodableList<Structs::ChargingTargetScheduleStruct::DecodableType> & newChargingTargetSchedules)
{
    ...
    while (newIter.Next())
    {
        ...
        PRINTF_DEBUG("SetTargets: DayOfWeekForSequence = 0x%02x", newBitmask);
        ...
        for (auto & currentChargingTargetSchedule : mChargingTargetSchedulesList)
        {
            ...
            PRINTF_DEBUG("SetTargets: Scanning current entry %d of %d: bitmap 0x%02x",
            ...
            if (currentBitmask == bitmaskA)
            {
                ...
                ReturnErrorOnFailure(
                    AllocAndCopyDailyTargetsWithLog(updatedChargingTargets, newChargingTargetSchedule.chargingTargets));
            ...
            else
            {
                ...
                ReturnErrorOnFailure(
                    AllocAndCopyDailyTargetsWithLog(updatedChargingTargets, currentChargingTargetSchedule.chargingTargets));
            }
        ...
        if (err != CHIP_NO_ERROR)
        {
            PRINTF_DEBUG("SetTargets: Failed to save Target to persistent storage %s", chip::ErrorStr(err));
            return err;
        }
        ...
        if (err != CHIP_NO_ERROR)
        {
            PRINTF_DEBUG("SetTargets: Failed to load Target from persistent storage %s", chip::ErrorStr(err));
            return err;
        }
    }
...
EvseTargetsDelegate::SaveTargets(DataModel::List<const Structs::ChargingTargetScheduleStruct::Type> & chargingTargetSchedulesList)
{
    ...
    {
        PRINTF_DEBUG("SaveTargets: DayOfWeekForSequence = 0x%02x",
    ...
    const DataModel::List<const Structs::ChargingTargetScheduleStruct::Type> & chargingTargetSchedules)
{
    ...
    for (auto & chargingTargetSchedule : chargingTargetSchedules)
    {
        [[maybe_unused]] uint8_t bitmask =
            chargingTargetSchedule.dayOfWeekForSequence.GetField(static_cast<TargetDayOfWeekBitmap>(kAllTargetDaysMask));
        PRINTF_DEBUG("idx %u dayOfWeekForSequence 0x%02x", chargingTargetScheduleIdx, bitmask);
        ...
        for (auto & chargingTarget : chargingTargetSchedule.chargingTargets)
        {
            ...
            PRINTF_DEBUG("chargingTargetIdx %u targetTimeMinutesPastMidnight %u targetSoC %u addedEnergy 0x" ChipLogFormatX64,
 ...
 **/
void EvseTargetsDelegate::OnFabricRemoved(const FabricTable &, FabricIndex)
{
    /*
     * Intentionally empty: EVSE charging targets are persisted under a single device key and are not
     * tied to a fabric index in this implementation, so fabric removal does not require clearing them.
     */
}
